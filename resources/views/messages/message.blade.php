...
<div class="bg-gray-900 h-screen">         
        <main class="rounded-lg shadow-lg overflow-hidden mx-auto max-w-sm pt-8 pb-2 lg:max-w-4xl">
            <div class="bg-yellow-200 rounded-lg my-0 px-4 py-6">
                <div class="md:flex rounded-lg my-0 md:items-center">
                    <div class="w-full h-64 md:w-1/2 lg:h-96">
                        <img class="h-full rounded-md max-w-lg mx-auto" src="https://images.unsplash.com/photo-1512820790803-83ca734da794?ixlib=rb-1.2.1&auto=format&fit=crop&w=689&q=80" alt="Nike Air">
                    </div>
                    <div class="w-full max-w-lg mx-auto md:w-1/2">
                        <h3 class="text-gray-900 uppercase text-lg font-bold">{{ $post->title }}</h3>
                        <span class="text-gray-900 mt-2">&#8369;{{ $post->price }}</span>
                        <hr class="my-4">
                        <div class="mt-2">
                            <label class="text-gray-700 text-sm" for="count"><b>Type:</b>&nbsp;&nbsp;  {{ $post->post_type }} - {{ $post->category }}<br><b>Description:</b><br>{{ $post->body }} </label>
                        </div>
                            ...
                            @if ((!is_null($post->pending_at)) and (is_null($post->completed_at)) and (auth()->user()->id === $post->bought_by_id))
                                ...
                                @if (session('status'))
                                    ...
                                    <form action="{{ route('transactions.user',auth()->user()->id) }}" method="get">
                                        <button href="" class="bg-blue-700 text-white hover:bg-blue-900 px-4 py-2 rounded
...
<!--message --> 
    <div class="pt-0 pb-8 bg-gray-900"> 
        <main class="rounded-lg shadow-lg overflow-hidden mx-auto max-w-sm lg:max-w-4xl">
            <div class=" hidden w-full bg-white h-full lg:flex flex-col justify-start border-r-2 border-l-2 border-gray-100 lg:rounded-r-md xl:rounded-none">
                <!-- Header with name -->
                <div class="flex flex-row items-center justify-between px-3 py-2 bg-yellow-200 bg-opacity-40 border-b-2 border-gray-100">
                    <div class="mx-2 my-2">
                        @if (auth()->user()->id === $post->bought_by->id)
                            <h2 class="text-gray-900  text-lg"> <b>Seller</b>   - {{ $post->sold_by->name }}</h2>
                        @else
                            <h2 class="text-gray-900  text-lg"> <b>Buyer</b>   - {{ $post->bought_by->name }}</h2>
                        @endif
                    </div>
                    <div class="flex flex-row">
                        <button type="button" class="p-2 ml-2 text-gray-900 rounded-full hover:text-gray-600 hover:bg-gray-100 focus:outline-none focus:ring" aria-label="Search">
                        ...
                        <button type="button" class="p-2 ml-2 text-gray-900 xl:text-blue-500 rounded-full hover:text-gray-600 hover:bg-gray-100 focus:outline-none focus:ring" aria-label="Open">
                        ...
                        <button type="button" class="p-2 ml-2 text-gray-9   00 rounded-full hover:text-gray-600 hover:bg-gray-100 focus:outline-none focus:ring" aria-label="More">
                            <svg class="w-6 h-6 fill-current" viewBox="0 0 24 24">
                ...
                <div class="flex flex-col justify-between">
                    <div class="flex flex-col">
                        <div class="flex h-screen p-2 px-4">
                            <div class="flex flex-1 justify-start overflow-y-scroll">
                                <ul class="p-4  break-all ">
                                            ...
                                            @else
                                                <h2 class="font-semibold">There are no messages</h2>
                                                @endif 
                ...
                <form action="{{ route('messages') }}" method="post" class="flex flex-row justify-between items-center p-3" >      
                    ...
                    <div class="flex flex-row">
                        <div>
                            <button type="submit" class="bg-blue-700 text-white hover:bg-blue-900 px-4 py-2 rounded font-medium">Send</button>
                        </div>
                    </div>
                </form>
            </div>
